...
// for open sidebar by click geer
var sidebar_collapse = true;
var sidebar_mode = 'op';
...
async function get_fleet_data(_fleet) {
    ...
    // var data_dict = JSON.parse(data); //strange maybe someone modify return data type
    var data_dict = data;
    var maps;
    var roles;
...
function save_planner_flow_setting(_fleetname, _flowname, _data) {
    ...
    restPostFlowData(_data, _fleetname, _flowname).done(function (data) {
        let msg = '';
        if (data === 'Flow planner saved') {
            // msg = `${_flowname} planner is saved`;
            notificationMsg(1, `${_flowname} planner saved`);
            dfd.resolve();
        } else {
            // msg = `${_flowname} planner is save fail`;
            notificationMsg(3, `${_flowname} planner saved failed`)
            dfd.resolve();
        }
        // toast(msg);
    });
...
function save(mode) {
    var save_trigger_mode = mode
    var save_flow_name = '';

    if (save_trigger_mode == 'op') {
        save_flow_name = flow_name;
    } else {
        save_flow_name = assign_robot_flow_name;
    }
    ...
    if ($('.drawflow-node.selected').length > 0) {
        if (!sidebar_collapse && save_trigger_mode === 'op') {
    ...
    var save_data = {
        "flow_name": save_flow_name,
        "event_name": event_name,
        "flow_data": flow_data_list
    }

    var trigger_name = $("#flow_select option:selected").text();
    var flow_filename = `${getSavedFleet()}-flow-${save_flow_name}`;
    var nameExists = true;
    restOperationFileExistence("flow", save_flow_name).done(function (data) {
        nameExists = data.fileExists && data.filename !== flow_filename;

        if (trigger_name.trim().length == 0) {
            alert("Please select a task trigger");
        } else if (nameExists) {
            alert(`${save_flow_name} already exists!`);
        } else {
            // console.log(editor.export());
            // console.log(JSON.stringify(save_data));
            // save to ui 
            if (save_trigger_mode == 'op') {
                save_ui_flow_setting(getSavedFleet(), save_flow_name, JSON.stringify(editor.export()));
                have_unsave_change = false;
            }
            ...
            save_planner_flow_setting(getSavedFleet(), save_flow_name, save_data).then(function () {
                dfd.resolve();
            });
        ...
        var mode = (isflow ? 2 : 1);
        if ($('#flow_select').val() === "manual" && mode === 2 && save_trigger_mode === 'op') {
...
function delete_flow() {
    ...
    notificationMsg(0, `${flow_name_cookie} is deleted`);
...
editor.on('nodeSelected', function (id) {
    ...
    $(`#node-${node_id}`).find('button').addClass(`detail_icon`);
    $(`#node-${node_id}`).find('button').attr('id', `detailid_node-${node_id}`);
    $(`#node-${node_id}`).find('button').bind("click", function () {
        sidebar_mode = 'op';
...
$(document).on("change", ".robot_ass", function () {
    ...
    if (selected_val === 'auto') {
        robot_select = '';
    } else {
        robot_select = selected_val;
    }
        ...
        case "goal":
                ...
                if (default_value.includes('@')) {
                    ...
                    for (let [map_item_key, map_item_val] of Object.entries(goal_cell_dict)) {
                        ...
                        if (map_name == map_item_key) {
                            map_option += `<option value="${map_item_key}" selected>${getMapAlias(map_item_key)}</option>`;
                            ...
                            map_option += `<option value="${map_item_key}">${getMapAlias(map_item_key)}</option>`;
                        }
                    };
            ...
            `;
                    ...
                    Object.keys(goal_cell_dict).forEach(function (content_item, content_index) {
                        if (content_index == 0) {
                            str_option += `<option value="${content_item}" selected>${getMapAlias(content_item)}</option>`;
                            ...
                            str_option += `<option value="${content_item}">${getMapAlias(content_item)}</option>`;
                        }
                    });
...
function generate_assign_robot_str(assign_robot, node_id) {
    var assign_robot_select = '';

    if (scannedAgents['total'] > 0) {
        assign_robot_select += `<option value="auto" selected>auto</option>`;
        scannedAgents['robots'].forEach(function (item, index) {
            var robot_id = item['robot_id'];
            var robot_name = item['robot_name'];
            if (assign_robot == robot_id) {
                assign_robot_select += `<option value="${robot_id}" selected>${robot_id}</option>`;
            } else {
                assign_robot_select += `<option value="${robot_id}">${robot_id}</option>`;
            }
        });
    } else {
        assign_robot_select += `<option value="auto" selected>auto</option>`;
...
function add_test_flow_btn() {
    var test_flow_str = `
			  <div class="col-12 text-center">
				<button type="button" class="btn btn-primary btn-lg" onclick="test_flow()">Test</button>
			  </div>
			  `;

    if (sidebar_collapse == false) {
        $('#card_content_div').append(test_flow_str);
    }
}
...
function testflowInit() {
    $('.drawflow-node').each(function (index) {
        if ($(this).hasClass('selected')) {
            ...
            console.log('Node no need to update.');
        }
    });
    console.log(flowdatadict)
    ...
    for (let [node_id, node_val] of Object.entries(flowdatadict)) {
        ...
        if (role_name != 'start' && role_name != 'finish' && role_name != 'logical_and' && role_name != 'logical_or' && role_name != 'logical_delay') {
            console.log('--------------------------------------')
            assign_robot_str = generate_assign_robot_str(assign_robot, node_id)
            card_template('t', `Role: ${role_name}`, role_name, assign_robot_str);
        }
    }
...
function test_flow() {
    ...
    save('test').then(function () {
        ...
        setTimeout(function () {
            ...
            // toast(`${assign_robot_flow_name} is deleted`);
            dfd.resolve();
        }, 1000);
    })
    return dfd.promise();
...
async function sendFlow() {
    ...
    if (res['status_code'] == 200) {
        ...
        // promptBox('success', 'Success: Flow Send!');
    } else {
        notificationMsg(3, `Flow Send Fail! ${res['message']}`);
        // promptBox('error', `Error: Flow Send Fail! ${res['message']}`);
    }
}

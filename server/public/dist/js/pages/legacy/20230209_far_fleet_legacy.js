...
//     title: 'Conveyor',
//     model: 'conveyor_06',
//     avatar: 'dist/img/sprites/conveyor-440x220.png',
//     thumbnail: 'dist/img/sprites/conveyor-150x150.png',
//   },
//   conveyor: {
//     type: 'conveyor_06',
//     title: 'Conveyor',
...
//     title: 'Top Module',
//     model: 'top_module',
//     avatar: 'dist/img/sprites/top_module-440x220.png',
//     thumbnail: 'dist/img/sprites/top_module-150x150.png',
//   },
//   juluen_top_module: {
//     type: 'juluen_top_module',
//     title: 'Juluen Top Module',
//     model: 'juluen_top_module',
//     avatar: 'dist/img/sprites/juluen_top_module-440x220.png',
//     thumbnail: 'dist/img/sprites/juluen_top_module-150x150.png',
//   },
//   lift_module: {
//     type: 'lift_module',
//     title: 'Lift Module',
//     model: 'lift_module',
//     avatar: 'dist/img/sprites/lift_module-440x220.png',
//     thumbnail: 'dist/img/sprites/lift_module-150x150.png',
//   },
//   lift_module_black: {
//     type: 'lift_module_black',
//     title: 'Lift Module Black',
//     model: 'lift_module_black',
//     avatar: 'dist/img/sprites/lift_module_black-440x220.png',
//     thumbnail: 'dist/img/sprites/lift_module_black-150x150.png',
//   },
//   lift: {
//     type: 'lift_module',
//     title: 'Lift Module',
...
//     avatar: 'dist/img/sprites/juluen_worktable-440x220.png',
//     thumbnail: 'dist/img/sprites/juluen_worktable-150x150.png',
//   },
//   tektro_worktable: {
//     type: 'tektro_worktable',
//     title: 'Tektro Work Table',
//     model: 'tektro_worktable',
//     avatar: 'dist/img/sprites/tektro_worktable-440x220.png',
//     thumbnail: 'dist/img/sprites/tektro_worktable-150x150.png',
//   },
//   far_robot_arm: {
//     type: 'far_robot_arm',
//     title: 'FAR Robot Arm',
//     model: 'far_robot_arm',
//     avatar: 'dist/img/sprites/far_robot_arm-440x220.png',
//     thumbnail: 'dist/img/sprites/far_robot_arm-150x150.png',    
//   },
// };
  ...
  $.validator.addMethod("chkValidFormat", function (value, element) {
    ...
    switch (type) {
      ...
      case 'string': {
        ...
        //     let match_str = match[0].replace('[', '');
        //     let footprint_val = match_str.split(',');
        //     console.log(footprint_val);
        //     if (footprint_val.length != 2){
        //       $.validator.messages.chkValidFormat = `value is not a correct format, please format`;
        //       return false;
        //     }else{
        //       footprint_val.forEach(v => {
        //         console.log(v)
...
async function switchFleetContext(_selFleet) {
  ...
  // console.log(scannedArtifacts);
  // console.log(artifactsProperty);
...
function loadFleetAgents(_data, _scanAgents) {
  ...
  // --- previous version ---
  // if (_scanAgents.length == 0) { return; }
  // _scanAgents.forEach((agent) => {
  //   let defaultAgent = {};
  //   defaultAgent["robot_id"] = agent.robot_id;
  //   defaultAgent["robot_name"] = agent.robot_name || "none";
  //   defaultAgent["model"] = agent.model || "none";
  //   defaultAgent["mode"] = agent.mode || "not available";
  //   defaultAgent["sw_version"] = agent.sw_version || "0.0.0";
  //   defaultAgent["ip"] = agent.ip || "127.0.0.1";
  //   defaultAgent["mac"] = agent.mac || "xx:xx:xx:xx:xx:xx";
  //   defaultAgent["fleet"] = agent.fleet_name || "none";
...
async function saveArtifactSettingsCb(e, _artifactId) {
  ...
  // var configData = `{'artifact_id': '${_artifactId}', 'artifact_name': 'test', 'type': 'conveyoer', 'sim': 'True', 'services': ${servicesStr}}`;
  ...
  try {
    ...
    if (response.status === 200) {
      notificationMsg(1, 'Artifact Settings are Set Successfully!');
    } else {
...
function avoidNoneVal(node, name, id) {
  let newName = '';
  if (name != undefined) {
    if ('none' == name.toLowerCase()) {
      newName = name + ' - ' + id;
    } else {
      newName = name;
    }
    ...
    console.log(`value is undifind name: ${name}, id: ${id}, node: ${node}`)
  }
}

function updateArtifactProfileView(_artifact) {
  // console.log(_artifact);
  $('#sb-agent-icon').attr('src', '/images/' + artifactAssets_[_artifact.type].thumbnail);
  ...
  if (servicesObj.hasOwnProperty('wms')) {
    ...
    artifactWmsCells.forEach(awc => {
      ...
      for (k in awc) {
        ...
        // var tag = (count % 2) ? `#${key}-left-col` : `#${key}-right-col`;
...
function updateArtifactConfActionView(_data) {
  ...
  importBtn.innerHTML = `<i class="fas fa-file-import"></i> ${langImportExportData[lang].import}`;
  ...
  exportBtn.innerHTML = `<i class="fas fa-file-export"></i> ${langImportExportData[lang].export}`;
...
async function updateSidebarAsyncCb(_agent) {
  // https://farobottech.atlassian.net/jira/software/projects/FAR/boards/2?assignee=624ea4cef6a269006961929e&selectedIssue=FAR-1385
  let name = document.getElementById('agent-artifact-name');
  name.querySelector("[data-i18n='fleet_config.sidebar.lbl_AgntArtfName']").textContent = langTemplateObj_.agnt.lbl_SidebarName;
  ...
  try {
    ...
    langData_ = await restGetAgentParamTranslation(currLocale_);
...
async function genSoftwareVerObj() {
  var tmpObj = {};
  try {
    var agentsVer = fleetVer.agents.agent_sw_version.map(agent => agent.sw_version);
    agentsVer = _.uniq(agentsVer);
    var agentVer = agentsVer.length === 1 ? agentsVer[0] : '';
    tmpObj.agent = agentVer;
    var artifactsVer = allFleetArtifacts_.map(artifact => artifact.sw_version);
    artifactsVer = _.uniq(artifactsVer);
    var artifactVer = artifactsVer.length === 1 ? artifactsVer[0] : '';
    tmpObj.artifact = artifactVer;
    var coreVer = await fetchGetCoreSwVer(rmtToken_);
...
async function updateAgentArtiReused(_updateId, _updateFunc, _progressFunc) {
  ...
  // var curVersion = document.getElementById('sb-agent-firmware');
  // if (curVersion.value == value) {
  //   alert(`Same version ${value}! No update needed.`);
  //   return
  // }
  // --- pop-up modal ---
...
function updateSaveButtonByMode(agentId) {
  let agentMode = $('#sb-agent-mode').val();
  if (agentMode === 'manual' || agentMode === 'maintain') {
    createSaveAgentButton(agentId);
  } else {
...
function boolButtonSwitch(e) {
  ...
  var stringVal = inputBoolVal.toString();
  var boolString = stringVal.charAt(0).toUpperCase() + stringVal.slice(1);
  var boolBtn = this.querySelector('.btn');
...
function setInputMapValue(mapSelect_) {
  var inputNode = this.querySelector('.form-control');
  inputNode.value = mapSelect_.value;
}

function addMapOption(mapSelect_, mapOpt, selMap) {
  var mapOption = document.createElement('option');
  mapOption.value = mapOpt;
  mapOption.text = mapOpt;
  mapOption.selected = (mapOpt === selMap);
  mapSelect_.add(mapOption);
}
...
async function saveFleetSettings(fleetName, oldFleetName) {
  ...
  const asyncAgentMapCheck = async (agentArray) => {
    for (const idx in agentArray) {
      ...
      // --- skip aligned case ---
      if (fltMaps.includes(runningMap) || runningMap === 'none') {
        continue;
      }
...
// const agentMessages_ = '{"far_sys": {"far_sys": {"initial_pose_x": "50.2105", "initial_pose_y": "37.1151", "initial_pose_yaw": "-1.57", "map": "jj_pdm_demo"}, "far_rmt_agent": {"robot_name": "none"}}, "far_controller": {"far_controller": {"PurePursuit": {"max_reference_speed": 1.0, "min_reference_speed": 0.5, "map_max_reference_speed": 0.3, "map_min_reference_speed": 0.2, "use_navigation": true, "use_map": false, "length_of_car": 1.2, "lfw": 0.6, "speed_incremental": 0.015, "rotation_speed_incremental": 0.05, "max_lookahead_distance": 1.6, "min_lookahead_distance": 1.0, "max_goal_radius": 0.1, "min_goal_radius": 0.05, "obstacle_dist_limitation": 3.0, "obstacle_achieve_time": 4.0, "max_angular_speed": 1.0, "map_max_angular_speed": 0.2}, "Docking": {"docking_time_limit": 60, "detect_time_limit": 3, "undocked_distance": 0.4, "average_point_num": 20, "toward_charger": false, "longitude_tolerance": 0.05, "lateral_tolerance": 0.03, "rotation_tolerance": 0.04}}}, "far_amcl": {"far_amcl": {"tf_broadcast": true, "laser_max_range": 15.0}}, "far_gmapping": {"far_gmapping": {"enable": false, "scan_topic": "scan_f"}}, "far_map_server": {"map_server": {"search_distance": 30.0}}, "far_robot_localization": {"ekf_filter_node": {"publish_tf": true}}, "far_global_scan_matching": {"far_global_scan_matching": {"ICP": {"use_map_limitation": false, "map_right_limitation": 9.0, "map_left_limitation": -1.0, "map_top_limitation": 0.0, "map_bottom_limitation": -6.5, "matching_time_limitation": 1.0, "epsilon": 0.0005}}}, "far_triton": {"far_triton": {"publish_tf": false}}, "far_local_planner": {"local_planner": {"robot_radius": 0.4, "footprint": "[[0.600,0.500],[0.600,-0.500],[-0.600,-0.500],[-0.600,0.500]]", "WaypointPlanner": {"max_global_plan_lookahead_dist": 2.0, "xy_goal_tolerance": 0.3, "yaw_goal_tolerance": 0.3}, "Teb": {"max_vel_x": 0.5, "max_vel_x_backwards": 0.2, "max_vel_y": 0.0, "max_vel_theta": 0.5, "xy_goal_tolerance": 0.4, "yaw_goal_tolerance": 0.4}}}, "accerion_driver": {"accerion_driver": {"ros_publishing/rel_tf": true, "ros_publishing/abs_tf": true}}, "far_landmark_recognition": {"landmark_recognition": {"RackDetection": {"detect_rack": false, "rack_length": 2.0, "rack_width": 2.0, "rack_epsilon": 0.1}, "ChargerDetection": {"charger_pattern_length": 0.4, "charger_pattern_angle": 2.09, "charger_pose_translation_x": 0.6, "charger_pose_translation_y": 0.2, "charger_pose_rotation": 0.0}, "QRCode": {"detector_type": 0, "Aruco": {"tag_sizes": [0.083], "tag_translation_x": [0.6], "tag_translation_y": [-0.05], "tag_rotation": [0.0]}, "Apriltag": {"tag_sizes": [0.073], "tag_translation_x": [0.0], "tag_translation_y": [0.0], "tag_rotation": [0.0]}}}}}';
...
function updateParamMembersView(_data, _langData) {
  var confData = _data;
  var paramObj = JSON.parse(confData);
  console.log(paramObj);
  ...
  for (var key in flattedParamObj) {
    ...
    var agentMode = $('#sb-agent-mode').val();
    var isEditMode = agentMode === 'manual' || agentMode === 'maintain';
    switch (key) {
      ...
      case "system.initial_pose_confirmed":
        pen_node.remove();
        enableConfirm = flattedParamObj[key];
      ...
      case "system.map":
        ...
        if (mapCacheArray.length > 0) {
          mapCacheArray.forEach((map) => {
            addMapOption(mapSelect, map, mapVal);
          });
          // --- protection for map name 'none' ---
          if (!mapCacheArray.includes(mapVal)) {
            addMapOption(mapSelect, mapVal, mapVal);
          }
        } else {
          addMapOption(mapSelect, 'none', mapVal);
        }

        mapSelect.addEventListener('change', setInputMapValue.bind(inputGroupNode, mapSelect));
...
function activateFirstActionView() {
  // --- activate first pane ---
  var sbFuncCard = document.getElementById('sidebar-func-card');
  var navTab = sbFuncCard.querySelector('.card-header > .nav-tabs');
  navTab.querySelector('.active')?.classList?.remove("active");
  var firstChild = getFirstChild(navTab).firstChild;
  // console.log(firstChild);
  firstChild?.classList?.add('active');
  var tabContent = sbFuncCard.querySelector('.card-body > .tab-content');
...
async function checkFleet() {
  ...
  var flows = fltSettings[fleetName]?.flows || [];
  var tasks = fltSettings[fleetName]?.tasks || [];
  var operation_text_arr = [];
...
function pollFleetStatesWithUpdates(_inteval = 1000) {
  ...
  setInterval(async function () {
    ...
    scannedAgents.forEach(function (item) {
      let id = item['robot_id'];
      robot_mode_dict[id] = item['mode'];
      robot_fleet[id] = item['fleet_name'];
      robot_name_dict[id] = item['robot_name'];
    });
    ...
    arrChildren.forEach(child => {
      ...
      avoidNoneName(titleNode, robot_name_dict[id], id);
      // console.log(id)

      let status_label = $(child).find('.agent-mode');
      let status_mode = 'not availiable';
      let colorStyle = 'red';
      if (robot_mode_dict.hasOwnProperty(id)) {
        status_mode = robot_mode_dict[id];
        if (status_mode == 'manual') {
          colorStyle = 'red';
        } else if (status_mode == 'maintain') {
          colorStyle = 'yellow';
        } else {
          colorStyle = '';
        }
      ...
      if (robot_fleet[id] != currFleet) {
        status_mode = 'Inconsistency';
        colorStyle = 'red';
      ...
      status_label.text(status_mode);

      // For sidebar update
      let current_sidebar_id = $('#sb-agent-id').val();
      if (current_sidebar_id == id && !openSidebar) {
        avoidNoneVal($('#sb-agent-name'), robot_name_dict[id], id);
        $('#sb-agent-mode').val(status_mode);

        if (current_sidebar_mode === status_mode) return;
        console.log(`${id} mode is change to ${status_mode}`);
        ...
        updateResetViewByMode(status_mode);
        activateFirstActionView();
        current_sidebar_mode = status_mode;
      }
    })
...
$(document).on('click', '.re_agent', async function () {
  ...
  var resetModal = document.getElementById('reboot-confirm-modal');
  var title = resetModal.querySelector('.modal-title');
  title.textContent = `Reboot ${agent_id}`

  var badge = resetModal.querySelector('.reset-target');
  badge.textContent = agent_id;

  var confirm = resetModal.querySelector('.reset-confirm');
  console.log(confirm);

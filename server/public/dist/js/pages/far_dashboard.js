...
let text_length = 20;
let robot_id_name_mapping_dict = {};
...
let avatarMap_ = agentThumbnailMapinfo(agentAssets_);

function agentThumbnailMapinfo(asset) {
  let mapping_dict = {};

  for (const [setting_key, setting_values] of Object.entries(asset)) {
    console.log(setting_values)
    mapping_dict[setting_key.toLowerCase()] = setting_values["thumbnail"];
  }
...
function updateFlowsWidgets3(_listView) {
  ...
  if (children.length !== 0) {
    children.forEach(c => {
      ...
      // --- existing job id ---
      var jobId = c.cells[0].lastChild.title.split('\n')[0].replace('ID: ', '');
      ...
      const newChild = document.createElement("span");
      const newChildText = document.createTextNode(c.cells[0].lastChild.previousSibling.textContent);
      newChild.appendChild(newChildText);
      c.cells[0].replaceChild(newChild, c.cells[0].lastChild.previousSibling);

      const newIconChild = document.createElement("i");
      newIconChild.className = "fa fa-info-circle";
      newIconChild.setAttribute('title', `ID: ${updatedInfo.id}\nName: ${updatedInfo.name}`);
      c.cells[0].replaceChild(newIconChild, c.cells[0].lastChild);
      ...
      c.cells[1].innerHTML = assigneeFormatter(updatedInfo.assignee, "", "");
      c.cells[2].innerHTML = statusFormatter(updatedInfo.status, "", "");
      c.cells[3].innerHTML = statusDetailFormatter(statusMsg, "", "");
...
function updateArtifactsWidgets3(_listView) {
  // --- data transformation ---
  let status = _.chain(_listView).map('status').countBy().value();
  ...
  _listView.forEach((artifact) => {
    ...
    if (artifact.hasOwnProperty('avatar')) {
      imgSrc = `data:image/png;base64, ${artifact.thumbnail}`;
    } else {
      ...
      imgSrc = artifactAssets_[model] === undefined? artifactAssets_['none'].thumbnail: artifactAssets_[model].thumbnail;
    }
    subNode.src = imgSrc;
...
function updateFleetWidgets3(_listView) {
  ...
  if (children.length) {
    for (let child of children) {
      ...
      let show_name_dict = adjustShowRobotName(target.robot_name, target.robot_id)
      robot_id_name_mapping_dict[target.robot_id] = show_name_dict["show_text"];
  ...
  _listView.forEach((agent) => {
    ...
    let subNode = node.querySelector('.agent-name');
    ...
    let model = agent.model.toLowerCase();
    // console.log(model);
    ...
    subNode.textContent = `${agent.task_id.slice(0, text_length)}...`;
    subNode.title = agent.task_id;
...
let isFolded_ = false;
function setupGraph() {
  const showStyle = (!isFolded_) ? 'major' : 'none';
  lcOptions_.xaxis.showTickLabels = showStyle;
  lcOptions_.yaxis.showTickLabels = showStyle;
...
function updateAgentModalContentFromFleetState(agentList) {
  ...
  let data = agentList.find(robot => robot.robot_id === selRobotID);
  if (!data) { return; }
...
$(document).ready(function () {
  ...
  $table.bootstrapTable({
    ...
    dataType: 'jsonp',
    columns: [
      { field: 'name', title: 'Name', align: 'left', formatter: 'nameFormatter' },
      { field: 'id', title: 'ID', visible: false, formatter: 'idFormatter' },
      ...
      { field: 'statusMsg', title: 'Status detail', sortable: true, formatter: 'statusDetailFormatter' },
...
function nameFormatter(value, row, index) {
  let show_name = value.length > 20 ? `${value.slice(0, 20)}...` : value;
  var idText = (row.id) ? row.id : "";
  var nameText = value ? value : "";
  var info = `&nbsp;<i title="ID: ${idText}\nName: ${nameText}" class="fa fa-info-circle"></i>`;
  return `<span>${show_name}</span>${info}`
}
...
function assigneeFormatter(value, row, index) {
  let show_name = robot_id_name_mapping_dict[value];
  let assigneeName = show_name === undefined ? value : show_name;
  assigneeName = assigneeName.length > 20 ? `${assigneeName.slice(0, 20)}...` : assigneeName;
  return `<div title="${assigneeName}">${assigneeName}</div>`
}
...
function statusFormatter(value, row, index) {
  ...
  return `<div align="left" value=${value}><span class="flow-status ribbon badge ${badge}">${val}</span></div>`;
}

// --- status detail formatter ---
function statusDetailFormatter(value, row, index) {
  let show_name = value.length > 25 ? `${value.slice(0, 25)}...` : value;
  return `<div title="${value}">${show_name}</div>`;
}
...
function pollFlowStates(_inteval = 1000) {
  setInterval(async function () {
    ...
    for (f of queryData.swarm_data) {
      var flow = { "id": f.flow_id, "pid": 0, "assignee": "-", status: f.state, statusMsg: "", "name": f.flow_name, "progressValue": f.complete_percent };
...
function pollFleetStates(_inteval = 1000) {
  setInterval(async function () {
    ...
    artifactData2_ = artifactData2_.map((artifact) => {
      let targetArtifact = artifactProperty.find(ap => ap.artifact_id === artifact.id);
      return (targetArtifact) ? { ...artifact, avatar: targetArtifact.avatar, thumbnail: targetArtifact.thumbnail } : artifact;
    });
...
function adjustPieChart() {
  const bIsDisplay = (window.innerWidth <= 1815 || window.outerWidth <= 1815) ? false : true;
  robotUtilChart.options.legend.display = bIsDisplay;
  planProgressChart.options.legend.display = bIsDisplay;
  taskPieChart.options.legend.display = bIsDisplay;
  artifactUtilChart.options.legend.display = bIsDisplay;

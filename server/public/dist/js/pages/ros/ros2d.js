...
ROS2D.ImageMapClient = function (options) {
	...
	// Send the request
	rosService.callService({}, function () {
...
ROS2D.OccupancyGridSrvClient = function (options) {
	...
	rosService.callService(new ROSLIB.ServiceRequest(), function (response) {
		...
		console.log("ready to get occupancy grid");
		that.currentGrid = new ROS2D.OccupancyGrid({
			message: response.map
		});
		that.rootObject.addChild(that.currentGrid);

		console.log("ready to emit change event");
...
/**
 * @author Shih-Chiang Wu - october.wu@gmail.com 
 */
 ...
 *   * pulse (optional) - if the marker should "pulse" over time
 */
ROS2D.FlatRectShape = function (options) {
	var that = this;
	options = options || {};
	var size = options.size || 1;
	var strokeSize = options.strokeSize || 1;
	// var strokeColor = options.strokeColor || createjs.Graphics.getRGB(0, 0, 0);
	var fillColor = options.fillColor || "lime";
...
ROS2D.FlatRectShape.prototype.__proto__ = createjs.Shape.prototype;
